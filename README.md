...
Given experimental data $y_{exp}$ with covariance $\{\Sigma_{\mathrm{exp}}\}$ and a simulator — either an explicit function $y(x)$ or a table of theory predictions $\{\boldsymbol{y}_{\mathrm{th}}(\boldsymbol{x}_n)\}$ — it samples the posterior of the parameters.
